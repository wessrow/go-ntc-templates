...
type CiscoNxosShowIpBgp struct {
...
var CiscoNxosShowIpBgp_Template = `# Status: s-suppressed, x-deleted, S-stale, d-dampened, h-history, *-valid, >-best
Value STATUS ([sxSdh*])
# ? \| multipath
...
Bgp_table
  ...
  ^\s*$$ -> Clearall
  ^BGP\s+routing\s.+VRF\s+${VRF},.+ -> Start
  ^. -> Error
